...
function initPage() {
  ...
  if (location.pathname.endsWith('obchodni-podminky.html')) termsPage();
  syncCart();
  registerTools();
}
document.addEventListener('click', (e) => {
...
function setDemoClient(value) {
  ...
  window.dispatchEvent(new CustomEvent('refurb:clientchange', { detail: { signedIn: value } }));
  toast(
    value
